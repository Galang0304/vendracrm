...
export default function EditStorePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.message || 'Gagal mengupdate toko')
      }
    } catch (error) {
      console.error('Error updating store:', error)
      alert('Terjadi kesalahan saat mengupdate toko')
    } finally {
      setSaving(false)
    }
  }
  ...
  if (loading) {
    return (
      <DashboardLayout title="Edit Toko">
    ...
    <DashboardLayout title="Edit Toko">
      <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Edit Toko</h1>
            <p className="text-sm sm:text-base text-gray-600">Update informasi toko</p>
          </div>
        </div>
